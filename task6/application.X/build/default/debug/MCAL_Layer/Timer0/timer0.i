...
# 34 "MCAL_Layer/Timer0/../GPIO/hal_gpio.h"
typedef enum {
    GPIO_LOW =0,
    GPIO_HIGH
}logic_t;
...
# 14 "MCAL_Layer/Timer0/../Interrupt/Interrupt_config.h"
# 1 "MCAL_Layer/Timer0/../Interrupt/Interrupt_gen_cfg.h" 1
# 14 "MCAL_Layer/Timer0/../Interrupt/Interrupt_config.h" 2
# 62 "MCAL_Layer/Timer0/../Interrupt/Interrupt_config.h"
typedef enum{
...
static __attribute__((inline)) void Timer0_Mode_select(const timer0_t *_timer){
    if(1 == _timer->timer0_mode ){
        ...
        (T0CONbits.T0CS =1);
        if(1 == _timer->timer0_counter_edge){(T0CONbits.T0SE = 0);}
        else if(0 == _timer->timer0_counter_edge){(T0CONbits.T0SE = 1);}
        else{ }
    }
    else { }
}
...
static __attribute__((inline)) void Timer0_register_size_config(const timer0_t *_timer){
    if(1==_timer->timer0_register_size){(T0CONbits.T08BIT =1);}
    else if(0==_timer->timer0_register_size){(T0CONbits.T08BIT =0);}
    else { }
}
